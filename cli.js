...
const { getEmbedPipeline, computeEmbedding, cosineSimilarity, forceEmbeddingsEnabled } = require('./src/core/embedding');
...
// ─── Code Index: symbol-level code knowledge (v7) ───────────────

commands['code-index'] = {
  desc: 'Code index: symbol-level code knowledge (ingest, search, embed, summarize, stats, top)',
  args: ['<ingest|search|embed|summarize|stats|top>', '[query]', '[--project PATH]', '[--limit N]', '[--semantic]', '[--changed-only]', '[--embed]', '[--summarize]', '[--body]'],
  parse(args) {
    const opts = {
      action: args[0] || 'stats',
      project: process.env.AGENTIC_CORTEX_PROJECT || process.cwd(),
      limit: 10,
      semantic: false,
      changedOnly: false,
      doEmbed: false,
    ...
    for (let i = 1; i < args.length; i++) {
      ...
      else if (args[i] === '--changed-only') opts.changedOnly = true;
      else if (args[i] === '--embed') opts.doEmbed = true;
      else if (args[i] === '--summarize') opts.doSummarize = true;
      else if (args[i] === '--body') opts.includeBody = true;
      else query.push(args[i]);
    }
    opts.query = query.join(' ');
    return opts;
  },
  async run(db, opts) {
    switch (opts.action) {
      case 'ingest': {
        let result;
        if (opts.changedOnly) {
          result = await api.codeIngestDiff(opts.project, {});
        } else {
          result = api.codeIngest(opts.project, { regenerate: true });
        }
        console.log(JSON.stringify(result, null, 2));
        if (opts.doEmbed) {
          console.log(JSON.stringify({ embed: await api.codeEmbed(opts.project, { limit: 200 }) }, null, 2));
        }
        if (opts.doSummarize) {
          console.log(JSON.stringify({ summarize: await api.codeSummarize(opts.project, { limit: 60 }) }, null, 2));
        }
        break;
      }
      case 'search': {
        if (!opts.query) {
          console.error('Usage: code-index search <query> [--project PATH] [--limit N] [--semantic] [--body]');
          process.exit(1);
        }
        // --semantic is an explicit opt-in to the ~400MB embedding model
        if (opts.semantic) {
          try { forceEmbeddingsEnabled(true); } catch { /* ignore */ }
        }
        const results = await api.codeSearch(opts.query, {
          project: opts.project, limit: opts.limit, semantic: opts.semantic, includeBody: opts.includeBody
        });
        console.log(JSON.stringify({ results, count: results.length }, null, 2));
        break;
      }
      case 'embed': {
        // Explicit embedding request — opt in to the model
        try { forceEmbeddingsEnabled(true); } catch { /* ignore */ }
        console.log(JSON.stringify(await api.codeEmbed(opts.project, { limit: opts.limit }), null, 2));
        break;
      }
      case 'top': {
        const results = api.codeTop(opts.project, { limit: opts.limit, withBody: opts.includeBody });
        console.log(JSON.stringify({ results, count: results.length }, null, 2));
        break;
      }
      case 'summarize': {
        console.log(JSON.stringify(await api.codeSummarize(opts.project, { limit: opts.limit }), null, 2));
        break;
      }
      case 'stats':
      default: {
        console.log(JSON.stringify(api.codeStats(opts.project), null, 2));
        break;
      }
    }
  }
...
commands.compact = {
  desc: 'Compact session context into a "state so far" summary (cuts conversation-history tokens)',
  args: ['[--session ID]', '[--transcript FILE]', '[--project PATH]', '[--save-observation]', '[--save-summary]'],
  parse(args) {
    const opts = { project: process.env.AGENTIC_CORTEX_PROJECT || process.cwd() };
    for (let i = 0; i < args.length; i++) {
      if (args[i] === '--session') opts.sessionId = args[++i];
      else if (args[i] === '--transcript') opts.transcript = args[++i];
      else if (args[i] === '--project') opts.project = args[++i];
      else if (args[i] === '--save-observation') opts.saveObservation = true;
      else if (args[i] === '--save-summary') opts.saveSummary = true;
    }
    return opts;
  },
  async run(db, opts) {
    if (opts.transcript) {
      const raw = JSON.parse(fs.readFileSync(opts.transcript, 'utf-8'));
      const entries = Array.isArray(raw) ? raw : (raw.entries || []);
      console.log(JSON.stringify(await api.compactTranscript(entries, {}), null, 2));
      return;
    }
    const result = await api.compactContext({
      sessionId: opts.sessionId || undefined,
      project: opts.project,
      saveObservation: opts.saveObservation,
